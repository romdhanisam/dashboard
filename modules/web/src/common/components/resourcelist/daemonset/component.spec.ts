...
import {DaemonSet, DaemonSetList, Event, Metric, PodInfo} from '@api/root.api';
...
describe('DaemonSetListComponent', () => {
  ...
  let daemonSet1: DaemonSet = {
    podInfo: undefined,
    initContainerImages: [],
    containerImages: ['nginx'],
    objectMeta: {name: 'daemonSet1'},
    typeMeta: undefined,
  };
  const daemonSetList: DaemonSetList = {
    cumulativeMetrics: [],
    listMeta: undefined,
  ...
  beforeEach(async () => {
    namespacedResourceServiceStub.get.mockReturnValue(of(daemonSetList));
    await TestBed.configureTestingModule({
      ...
      providers: [
        {provide: NamespacedResourceService<DaemonSetList>, useValue: namespacedResourceServiceStub},
  ...
  it('should create endpoint', () => {
    const expectedURI: string = 'api/v1/daemonset/:namespace';
  ...
  it('hasError should return false', () => {
    // Given
    daemonSet1 = {...daemonSet1, podInfo: {warnings: []} as PodInfo};
    // When
    const hasError: boolean = component.hasErrors(daemonSet1);
    // Then
  ...
  it('hasError should return true', () => {
    // Given
    daemonSet1 = {...daemonSet1, podInfo: {warnings: [{} as Event]} as PodInfo};
    // When
    const hasError: boolean = component.hasErrors(daemonSet1);
    // Then
