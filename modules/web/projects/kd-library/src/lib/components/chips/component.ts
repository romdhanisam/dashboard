...
import {KdStateService} from '../../global/state';
...
@Component({
  selector: 'kd-legacy-chips',
  styleUrls: ['./style.scss'],
  templateUrl: './template.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ChipsV2Component implements OnInit, OnChanges {
  @Input({ required: true }) map: { [key: string]: string }| string[] | number[];
  @Input({ required: true }) displayAll = false;
  @Output() onShowDetails: EventEmitter<{key: string, value: string }> = new EventEmitter();
  keys: string[];
  isShowingAll = false;
  @Input({ required: true }) private labelsLimit : number;

  constructor(private readonly _kdStateService: KdStateService,
  ...
  isVisible(index: number): boolean {
    return this.isShowingAll || index < this.labelsLimit || this.displayAll;
  }

  isAnythingHidden(): boolean {
    return this.keys.length > this.labelsLimit && !this.displayAll;
  }
  ...
  openChipDialog(key: string, value: string): void {
    this.onShowDetails.emit({key, value});
  }
